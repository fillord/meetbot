...
async def process_name_input(message: Message, state: FSMContext, db: Database):
    ...
    # Переходим к вводу возраста
    await state.set_state(ProfileStates.waiting_for_age)
    await message.answer(
        "🎂 <b>Сколько тебе лет?</b>\n\n"
        f"Укажи свой возраст (от {Config.MIN_AGE} до {Config.MAX_AGE} лет):",
        parse_mode="HTML",
        reply_markup=get_back_to_menu_keyboard()
    )
...
async def process_age_input(message: Message, state: FSMContext, db: Database):
    ...
    # Переходим к получению геолокации
    await state.set_state(ProfileStates.waiting_for_location)
    await message.answer(
        "📍 <b>Укажи свое местоположение</b>\n\n"
        "Для поиска людей рядом с тобой нужно знать твоё местоположение.\n"
        "Нажми кнопку ниже, чтобы отправить геолокацию 👇\n\n"
        "⚠️ <i>Этот шаг обязателен для работы с ботом</i>",
        parse_mode="HTML",
...
async def process_location_input(message: Message, state: FSMContext, db: Database):
    ...
    if message.location:
        ...
        await message.answer(
            "📖 <b>Расскажи о себе</b>\n\n"
            f"Напиши немного о себе (от 10 до {Config.MAX_BIO_LENGTH} символов).\n"
            "Что тебе нравится? Чем занимаешься? Что ищешь в отношениях?\n\n"
            "⚠️ <i>Это поле обязательно для заполнения</i>",
            parse_mode="HTML",
            reply_markup=get_back_to_menu_keyboard()
        )
    else:
        # Если не геолокация, то просим еще раз
        await message.answer(
...
async def process_city_input(message: Message, state: FSMContext, db: Database):
    """Обработка ввода города (устаревший обработчик, перенаправляем на геолокацию)"""
    await message.answer(
...
async def process_bio_input(message: Message, state: FSMContext, db: Database):
    ...
    # Проверяем контент с помощью AI
    ...
    await message.answer(
        "📸 <b>Добавь свое фото</b>\n\n"
        "Пришли фотографию, которая будет главной в твоей анкете.\n"
        f"Можно добавить до {Config.MAX_PHOTOS_PER_PROFILE} фотографий.",
        parse_mode="HTML",
        reply_markup=get_back_to_menu_keyboard()
    )
...
async def finalize_profile_creation(message, state: FSMContext, db: Database):
    ...
    try:
        ...
        # Если не удалось показать профиль с фото, показываем без фото
        # Fallback handled by show_user_profile
...
async def show_profile_without_photo(message, db: Database, user_id: int):
    """Показать профиль без фотографии"""
    user = await db.get_user(user_id)
    if not user:
        await message.answer(
            "❌ Анкета не найдена",
...
async def show_user_profile(message: Message, db: Database, user_id: int, is_own: bool = False, edit_message: bool = False):
    ...
    # Получаем главное фото (первое в списке уже отсортированном по is_main DESC)
    photos = await db.get_user_photos(user_id)
